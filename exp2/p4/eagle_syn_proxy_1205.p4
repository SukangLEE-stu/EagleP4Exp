...
// 寄存器配置
#define REGISTER_SIZE (bit<32>)4096
...
control MyIngress(inout headers hdr,
    ...
    action calc_reg_index_rev() {
        hash(meta.reg_index, HashAlgorithm.crc32, (bit<32>)0, {hdr.ipv4.dstAddr,
                                                           hdr.ipv4.srcAddr,
                                                           hdr.tcp.dstPort,
                                                           hdr.tcp.srcPort,
                                                           hdr.ipv4.protocol},
    ...
    action forward_server_to_client() {
        ...
        // standard_metadata.egress_spec = 1; // 客户端端口
        }
    }

     // action send_syn_ack_to_client() {
     //
     // }

     // action initiate_server_connection() {

    // }

     // action send_ack_to_server() {
     // }
    ...
    apply {
        ...
        // todo in furture, this should be surrounded with threshold trigger
            ...
            // 客户端ACK验证
            else if (hdr.tcp.ctrl == TCP_ACK && meta.direction == 0) {
            ...
            // 服务器SYN-ACK处理
            else if (hdr.tcp.ctrl == TCP_SYN_ACK && meta.direction == 1) {
                // todo this meta.direction is unnessceary because SYN_ACK only happens in
                // todo     stream from server to client, c -> s don't send this packet
                handle_server_syn_ack();
                mac_port_table.apply();
            }
            // 已建立连接的转发
            // else if (meta.direction == 0) { // 客户端到服务器
            //     forward_client_to_server();
            // }
            // else if (meta.direction == 1) { // 服务器到客户端
            //     forward_server_to_client();
            // }
            else {
                // todo forward fucntion should act in if ACK part
                forward_server_to_client();
                mac_port_table.apply();
            }
        }
