...
def find_feature_split(model, tree_index, num_features):
    ...
    for l in range(num_features):
        ...
        first_letter = int(np.floor(l / 24))
...
def generate_feature_tables(split, num_features, feature_max, table):
    for i in range(num_features):
        # print('len check:', len(num_features), len(feature_max))
        table["feature " + str(i)] = {}
        count_code = 0
...
# def generate_feature_tables(split, num_features, feature_max, table):
#     for i in range(num_features):
#         thresholds = sorted(split["feature " + str(i)])
#         table["feature " + str(i)] = {}
#
#         # 构造分段编码规则：value -> code
#         code = 0
...
#             start_threshold = thresholds[idx]
#             end_threshold = thresholds[idx + 1]
#
#             # 处理左边界
#             if start_threshold == -np.inf:
#                 start = 0  # 可以设置为最小值或根据数据范围调整
#             else:
#                 start = int(np.ceil(start_threshold))
#
#             # 处理右边界
#             if end_threshold == np.inf:
#                 end = int(feature_max[i])  # 使用特征最大值作为上限
#             else:
#                 end = int(np.floor(end_threshold))
#
#             # 避免无效区间
#             if start > end:
#                 continue
...
def generate_table(model, tree_index, num_features, g_table, feature_max):
    ...
    for i in range(num_features):
        code_width_for_feature[i] = int(np.ceil(math.log(
            g_table[tree_index]['feature ' + str(i)][np.max(list(g_table[tree_index]['feature ' + str(i)].keys()))] + 1,
            2))) or 1
    print('cp4')
    g_table[tree_index] = generate_code_table(g_table[tree_index], path_to_leaf, num_features)

    print('\rThe table for Tree: {} is generated\n'.format(tree_index), end="")
    return g_table
...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    for i in range(num_features):
        ...
        for value in range(feature_max[i]):
            tmp = []
            for tree in range(num_trees):
                tmp += [g_table[tree]["feature " + str(i)][value]]
            Ternary_Table['feature ' + str(i)].append(tmp)
    # print("transfer table is", Ternary_Table)

    Exact_Table = copy.deepcopy(Ternary_Table)
    for i in range(num_features):
        Exact_Table['feature ' + str(i)] = {}
        for value in range(feature_max[i]):
            Exact_Table['feature ' + str(i)][value] = []
            for tree in range(num_trees):
                Exact_Table['feature ' + str(i)][value] += [g_table[tree]["feature " + str(i)][value]]
